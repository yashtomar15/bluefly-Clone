...
{
    if(text[0]==="B"){
        axios.get(`https://blueflyapp.herokuapp.com/Data/filter?${sortCategory[1]}trendingcat=best_seller`)
        ...
        axios.get(`https://blueflyapp.herokuapp.com/Data/${sortCategory[0]}?title=inc`)
          .then(({data})=>{filterData(data); console.log(data)})
          .catch((err)=>{console.log("err occured: ", err)});
        setValue('Title: A-Z');
    }
    else if(text[text.length-1]==="A"){
        axios.get(`https://blueflyapp.herokuapp.com/Data/${sortCategory[0]}?title=dec`)
          .then(({data})=>{filterData(data);})
          .catch((err)=>{console.log("err occured: ", err)});
        setValue('Title: Z-A');
    }
        ...
        axios.get(`https://blueflyapp.herokuapp.com/Data/${sortCategory[0]}?price=inc`)
          .then(({data})=>{filterData(data);})
          .catch((err)=>{console.log("err occured: ", err)});
        setValue('Price: Low To High');
    }
    else if(text[text.length-1]==="W"){
        axios.get(`https://blueflyapp.herokuapp.com/Data/${sortCategory[0]}?price=dec`)
          .then(({data})=>{filterData(data);})
          .catch((err)=>{console.log("err occured: ", err)});
        ...
        axios.get(`https://blueflyapp.herokuapp.com/Data/filter?${sortCategory[1]}trendingcat=Trending`)
        .then(({data})=>{filterData(data);})
        .catch((err)=>{console.log("err occured: ", err)});
        setValue('Trending');
    }
